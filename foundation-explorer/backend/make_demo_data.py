...
Sample: top 1,500 foundations by confirmed Christian giving + 500 others.
...
from db import get_conn, is_v2_pipeline, rows_to_dicts
from evidence_queries import christian_evidence as evidence_query
from recipient_queries import list_recipients
...
def christian_evidence(conn, ein):
    """Christian recipients from the exact release behind the verdict."""
    return evidence_query(conn, ein)["recipients"][:30]
...
def sample_eins(conn) -> list[str]:
    ...
    for pat in MARQUEE_NAMES:
        marquee.update(
            r[0]
            for r in conn.execute(
                "SELECT ein FROM universe WHERE foundation_name LIKE ?", (f"%{pat}%",)
            )
        )
    ...
    rand = [
        ...
        for r in conn.execute(
            ...
            f"({','.join('?' * len(seen))}) ORDER BY ein LIMIT 500",
            list(seen),
        )
    ]
...
def detail_bundle(conn, ein: str) -> dict:
    bare = ein.lstrip("0")
    grants = rows_to_dicts(
        conn.execute(
            "SELECT grantee_name,city,state,country,is_foreign,amount,purpose,tax_year "
            "FROM pipeline.grants WHERE ein IN (?,?) ORDER BY amount DESC LIMIT ?",
            (ein, bare, GRANTS_PER_FOUNDATION),
        ).fetchall()
    )
    totals = conn.execute(
        "SELECT COUNT(*),SUM(amount) FROM pipeline.grants WHERE ein IN (?,?)",
        (ein, bare),
    ).fetchone()
    recipients = rows_to_dicts(
        conn.execute(
            "SELECT grantee_name,COUNT(*) grant_count,SUM(amount) total_amount,"
            "COUNT(DISTINCT tax_year) years FROM pipeline.grants WHERE ein IN (?,?) "
            "GROUP BY grantee_name ORDER BY total_amount DESC LIMIT 20",
            (ein, bare),
        ).fetchall()
    )
    attach_legacy_tags(conn, recipients)
    activities = rows_to_dicts(
        conn.execute(
            "SELECT description,expenses,tax_year FROM pipeline.charitable_activities "
            "WHERE ein IN (?,?) ORDER BY tax_year DESC,expenses DESC LIMIT 10",
            (ein, bare),
        ).fetchall()
    )
    return {
        "grants": grants,
        "grants_total": totals[0],
        "grants_dollars": totals[1],
        "recipients": recipients,
        "activities": activities,
        "christian_evidence": christian_evidence(conn, ein),
    }


def attach_legacy_tags(conn, recipients: list[dict]) -> None:
    ...
    for row in recipients:
        ...
        row["tags"] = json.loads(found[0]) if found else []
...
def main():
    ...
    grants_sample = rows_to_dicts(
        conn.execute(
            ...
            f"ORDER BY g.amount DESC LIMIT 5000",
            eins + bare_eins,
        ).fetchall()
    )
    names = dict(
